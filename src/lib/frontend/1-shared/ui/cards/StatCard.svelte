<script lang="ts">
	...
	const cardClass = $derived(
		accent === 'amber'
			? 'bg-accent-tint ring-accent-sm'
			: accent === 'red'
				? 'bg-red-900/20 ring-1 ring-red-800/40'
				: 'bg-stone-800'
	);
	const labelClass = $derived(
		accent === 'amber' ? 'text-accent' : accent === 'red' ? 'text-red-500' : 'text-stone-500'
	);
...
{#if onclick}
	<button
		type="button"
		class={[
			'flex w-full cursor-pointer flex-col rounded-2xl p-5 text-left transition hover:brightness-110 active:scale-95',
			cardClass
		]}
		{onclick}
	>
		...
		{:else}
			<span
				...
				style={valueColor ? `color: ${valueColor}` : ''}>{value}</span
			>
		{/if}
