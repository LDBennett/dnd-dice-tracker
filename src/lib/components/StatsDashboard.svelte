<script lang="ts">
	...
	let { totalRolls, averageValue, nat20s, nat1s, isGuest }: Props = $props();
...
<div class="px-4 py-6">
	...
	<div class="grid grid-cols-2 gap-4">
		<div class="flex flex-col rounded-2xl bg-slate-800 p-5">
			<span class="mb-1 text-xs font-semibold tracking-widest text-slate-500 uppercase"
				>Total Rolls</span
			>
		...
		<div class="flex flex-col rounded-2xl bg-amber-400/10 p-5 ring-1 ring-amber-400/30">
			<span class="mb-1 text-xs font-semibold tracking-widest text-amber-500 uppercase"
				>Nat 20s</span
			>
		...
		<div class="flex flex-col rounded-2xl bg-red-900/20 p-5 ring-1 ring-red-800/40">
			<span class="mb-1 text-xs font-semibold tracking-widest text-red-500 uppercase">Nat 1s</span>
	...
	{:else if nat20s > 0 || nat1s > 0}
		<div class="mt-6 rounded-2xl bg-slate-800 p-4 text-sm text-slate-400">
			{#if nat20s > 0 && nat1s > 0}
				<p>
					You've rolled {nat20s} natural 20{nat20s !== 1 ? 's' : ''} and {nat1s} natural 1{nat1s !==
					1
						? 's'
						: ''}.
				</p>
