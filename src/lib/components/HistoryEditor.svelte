		...
		try {
			...
			if (res.ok) {
				...
				rolls  = Object.fromEntries(
					sessions.map((s) => [s.id, s.rolls.map((r) => ({ ...r }))])
				);
			}
		} finally {
			loading = false;
				...
				<div class="rounded-2xl bg-slate-800 p-4">
					...
					<div class="relative mb-3">
						<input
							...
							class="w-full rounded-xl border border-slate-600 bg-slate-700/60 px-3 py-2 text-sm font-semibold text-white placeholder-slate-500 focus:border-amber-400 focus:outline-none focus:ring-1 focus:ring-amber-400/30"
						/>
					</div>
					...
					<div class="flex flex-col gap-2">
						{#each rolls[session.id] ?? [] as roll, i}
							<div class="flex items-center gap-2">
								...
								<input
									...
									placeholder="Note…"
									class="min-w-0 flex-1 rounded-lg border border-slate-600 bg-slate-700/60 px-2.5 py-1.5 text-xs text-white placeholder-slate-500 focus:border-amber-400 focus:outline-none focus:ring-1 focus:ring-amber-400/30"
								/>
							</div>
						{/each}
